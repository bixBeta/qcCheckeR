  ...
  navbarPage(
    ...
    tabPanel(
      ...
      fluidPage(
        ...
        card(
          ...
          card_body(
            p("Enter your email to access your saved sessions across all pipeline apps."),
            fluidRow(
              column(6,
                textInput(
                  "user_email",
      ...
      fluidPage(
        ...
        card(
          ...
          card_body(
            ...
            tags$p(
              ...
              tags$em("Warning: If you close the browser with unsaved changes, you'll see a confirmation dialog.")
            )
          )
        ),
        br(),
        ...
        tags$ol(
          ...
          tags$li(
            ...
            "Go to the ", tags$em("Annotate Counts"), " tab to add gene names and descriptions. ",
    ...
    tabPanel(
      ...
      fluidPage(
        br(),
        h4("Gene Annotation"),
        p("Add gene names and descriptions to your count matrix. Choose between g:Profiler (online) or a local GTF file.",
          style = "color: #666;"),
            ...
            card_body(
              ...
              fluidRow(
                column(6,
                  ...
                  p("Supports .gtf, .gff, .gff3 (and gzipped versions)", 
          ...
          card(
            card_body(
              div(
                ...
                p("Upload data in the Upload/Rename tab, then use g:Profiler or upload a GTF file to add gene names and descriptions.")
              )
            )
          )
        )
                  ...
                  nav_panel(
                    ...
                    page_sidebar(
                      ...
                      div(
                        ...
                        withSpinner(
                          plotlyOutput("topgenes_heatmap", height = "1200px", width = "840px"),
                          type = 4,
                          color = "#0dcaf0"
                        )
                      )
...
server <- function(input, output, session) {
  ...
  observeEvent(user_rv$current_session_id, {
    ...
    if (isolate(user_rv$authenticated)) {
      ...
      tryCatch({
        log_session_activity(
          ...
          samples = if (!is.null(isolate(data_rv$meta))) nrow(isolate(data_rv$meta)) else NA,
      ...
      card_body(
        if (!is.null(user_rv$current_session_id)) {
          tagList(
            p(
              strong("Session ID: "),
              tags$code(user_rv$current_session_id, style = "font-size: 0.85em;")
            ),
            if (!is.null(data_rv$meta)) {
              p(
                class = "small text-muted",
                icon("table"), " ", nrow(data_rv$meta), " samples loaded"
              )
            }
          )
        } else {
  ...
  observeEvent(input$copy_session_id, {
    ...
    showNotification("Session ID copied to clipboard!", type = "message", duration = 2)
  })
  ...
  output$send_to_deg_explorer_ui <- renderUI({
    ...
    div(
      ...
      tags$a(
        href = paste0(deg_explorer_url, "?session=", user_rv$current_session_id, 
                      "&email=", URLencode(user_rv$email)),
        target = "_blank",
  ...
  generate_session_id <- function() {
    paste0("qc_", format(Sys.time(), "%Y%m%d%H%M%S"), "_", 
  ...
  do_save_session <- function() {
    ...
    tryCatch({
      ...
      current_meta <- if (!is.null(meta_module_out) && !is.null(meta_module_out$data)) {
      ...
      session_data <- list(
        ...
        created = Sys.time(),
        updated = Sys.time(),
        file_name = user_rv$current_file_name %||% isolate(input$file$name) %||% "data.RData",
        file_data = list(
          metadata = data_rv$orig_meta,
          counts = data_rv$orig_counts
        ),
      ...
      # Save to file
      session_dir <- get_user_session_dir(SESSIONS_BASE_PATH, user_rv$email, APP_NAME)
      ...
      session_file <- file.path(session_dir, paste0(user_rv$current_session_id, ".rds"))
  ...
  observeEvent(input$modal_load_session, {
    ...
    tryCatch({
      ...
      if (!file.exists(session_file)) {
        showNotification("Session file not found", type = "error", duration = 5)
        return()
      }
 ...
 observeEvent(input$start_new, {
  ...
  output$annotation_source_badge <- renderUI({
    ...
    if (annotation_rv$method == "gprofiler") {
      span(
        ...
        style = "font-size: 0.85em;",
        icon("cloud"), " g:Profiler"
      )
    } else if (annotation_rv$method == "gtf") {
      span(
        class = "badge bg-success",
        style = "font-size: 0.85em;",
        icon("file-code"), " GTF File"
      )
    }
  })
  ...
  observeEvent(input$run_annotation, {
    ...
    tryCatch({
      ...
      progress$set(value = 0.2, detail = paste0("Querying g:Profiler for ", length(gene_ids), " genes..."))
      ...
      showNotification(
        paste0("Annotation complete! ", n_annotated, " of ", nrow(counts_with_anno), " genes annotated."),
        type = "message",
        duration = 5
      )
  ...
  observeEvent(input$run_gtf_annotation, {
    ...
    tryCatch({
      ...
      progress$set(value = 0.5, detail = paste0("Processing ", length(gene_lines), " features..."))
      ...
      progress$set(value = 0.7, detail = paste0("Found ", nrow(gtf_data), " unique genes..."))
      ...
      counts_with_anno <- counts_rv$counts %>%
        ...
        left_join(
          gtf_lookup %>% select(clean_id, gene_name, description, coordinates),
          by = "clean_id"
        ) %>%
        mutate(ensembl_id = gene_id) %>%  # Use gene_id as ensembl_id for GTF
        select(gene_id, ensembl_id, gene_name, description, coordinates, everything(), -clean_id)
      ...
      showNotification(
        paste0("GTF annotation complete! ", n_annotated, " of ", nrow(counts_with_anno), " genes annotated."),
        type = "message",
        duration = 5
      )
  ...
  output$download_pca_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      plot_title <- if (!is.null(input$plot_title) && input$plot_title != "") input$plot_title else "PCA Score Plot"
      plot_xlabel <- sprintf("PC%d (%.1f%% variance)", pc_x, var_explained[pc_x])
      plot_ylabel <- sprintf("PC%d (%.1f%% variance)", pc_y, var_explained[pc_y])
      point_size <- if (!is.null(input$plot_point_size)) input$plot_point_size / 5 else 4  # Scale for ggplot
      ...
      if (format == "html") {
        ...
        p <- ggplot(plot_data, aes(x = .data[[pc_x_col]], y = .data[[pc_y_col]], color = color_var)) +
          ...
          theme(
            ...
            plot.background = element_rect(fill = "white", color = NA),
            panel.grid.major = if (show_grid) element_line(color = "#e0e0e0", linewidth = 0.3) else element_blank(),
            panel.grid.minor = element_blank(),
        ...
        if (format == "png") {
          ggsave(file, plot = p, width = width, height = height, dpi = 150, bg = "white")
        } else if (format == "pdf") {
          ggsave(file, plot = p, width = width, height = height, device = "pdf", bg = "white")
        } else if (format == "svg") {
          ggsave(file, plot = p, width = width, height = height, device = "svg", bg = "white")
        }
        
        removeModal()
        showNotification(paste0("Plot downloaded as ", toupper(format)), type = "message", duration = 3)
      }
    }
  )
 ...
 # Update PC choices when PCA is computed (for both tabs)
  ...
  top_loading_genes <- reactive({
    ...
    if (isTRUE(annotation_rv$ready) && !is.null(annotation_rv$annotated_data)) {
      ...
      has_coordinates <- "coordinates" %in% colnames(annotation_rv$annotated_data)
  ...
  output$topgenes_heatmap <- renderPlotly({
    ...
    pc_loadings <- loadings[, pc_idx]
    top_idx <- order(abs(pc_loadings), decreasing = TRUE)[1:min(n_genes, length(pc_loadings))]
    ...
    if (isTRUE(annotation_rv$ready) && !is.null(annotation_rv$annotated_data)) {
      ...
      if (any(duplicated(new_rownames))) {
        dup_idx <- which(duplicated(new_rownames) | duplicated(new_rownames, fromLast = TRUE))
        new_rownames[dup_idx] <- paste0(new_rownames[dup_idx], " (", rownames(vst_subset)[dup_idx], ")")
      }
    ...
    scale_rows <- if (is.null(input$heatmap_scale_rows)) TRUE else input$heatmap_scale_rows
    cluster_rows <- if (is.null(input$heatmap_cluster_rows)) FALSE else input$heatmap_cluster_rows
    cluster_cols <- if (is.null(input$heatmap_cluster_cols)) FALSE else input$heatmap_cluster_cols
    show_dendro <- if (is.null(input$heatmap_show_dendro)) TRUE else input$heatmap_show_dendro
    show_rownames <- if (is.null(input$heatmap_show_rownames)) TRUE else input$heatmap_show_rownames
    show_colnames <- if (is.null(input$heatmap_show_colnames)) TRUE else input$heatmap_show_colnames
    ...
    # Color palette
    color_pal <- colorRampPalette(rev(RColorBrewer::brewer.pal(11, "RdBu")))(100)
    if (heatmap_colors == "viridis") color_pal <- viridisLite::viridis(100)
    if (heatmap_colors == "PRGn") color_pal <- colorRampPalette(rev(RColorBrewer::brewer.pal(11, "PRGn")))(100)
    if (heatmap_colors == "RdYlBu") color_pal <- colorRampPalette(rev(RColorBrewer::brewer.pal(11, "RdYlBu")))(100)
    if (heatmap_colors == "plasma") color_pal <- viridisLite::plasma(100)
    ...
    annotation_cols <- setNames(
      colorRampPalette(c("#636EFA", "#EF553B", "#00CC96", "#AB63FA", "#FFA15A", "#19D3F3", "#FF6692", "#B6E880"))(n_levels),
    ...
    main_title <- paste0("Top ", nrow(vst_subset), " Loading Genes - ", pc_name,
                         if (scale_rows) " (Z-score)" else " (VST)")
    
    # Determine dendrogram setting
    dendro_setting <- "none"
    if (show_dendro) {
      if (cluster_rows && cluster_cols) dendro_setting <- "both"
      else if (cluster_rows) dendro_setting <- "row"
      else if (cluster_cols) dendro_setting <- "column"
    }
